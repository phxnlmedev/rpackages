...
assign("NlmeCovarItem",NlmeCovarItem,env=.GlobalEnv)
...
#' @param continuousType   Type of value to use for the centering value: CovarNumber|CovarMean|CovarMedian
#' @param covarItems
#' @param covarEffList
#'
#' @examples
...
assign("NlmeCovariateParameter",NlmeCovariateParameter,env=.GlobalEnv)
...
#'
print.NlmeCovariateParameter <-function(obj)
{
    type = attr(obj,"type")
    if ( type == COVAR_CONTINUOUS ) {
        print(paste(attr(obj,"name"), " : Continuous"))
        print(paste("Direction : ",CovarDirectionNames[attr(obj,"direction")]))
        print(paste("Center    : ",attr(obj,"centerValue")))
        print(paste("Type      : ",CovarEvalNames[attr(obj,"continuousType")]))
        print(paste("Positive? : ",attr(obj,"isPositive")))
    } else if ( type == COVAR_CATAGORY ) {
        print(paste(attr(obj,"name"), " : Category"))
        print(paste("Direction : ",CovarDirectionNames[attr(obj,"direction")]))
        l=attr(obj,"covarItems")
        for ( c in l )
            print(paste0(attr(c,"name"), " " , attr(c,"value")))
    } else if ( type == COVAR_OCCASION ) {
        print(paste(attr(obj,"name"), " : Occasion"))
        print(paste("Direction : ",CovarDirectionNames[attr(obj,"direction")]))
        l=attr(obj,"covarItems")
        for ( c in l )
            print(paste0(attr(c,"name"), " " , attr(c,"value")))
    }
    print(attr(obj,"covarEffList"))
    print("")
}
...
#' occasionCovariate
#'
#' Convenience method to create a occasion covariate
#'
#' @param name           Name of the covariate
